...
const {statics, md5: {md5Hash}} = require('../lib');
...
const staticProtoRoot = path.join(cwd, '../storage', 'static/protos');
...
describe('server', function () {
  ...
  console.log({staticProtoRoot, filepath})
  const fullpath = path.join(staticProtoRoot, filepath);
  before('initialization', async function () {
    server = await startServer({protoStaticFilesRoot: staticProtoRoot});
  ...
  describe('.compareSums()', function () {
    const compareSums = async (sumfile) => {
    ...
    it('should repond comparison between loacal and remote', async function () {
      const types = 'gnat/gnater/types.proto';
      const sumfile = `
${filepath}:${expectedSum}
${testProto.filepath}:${testProto.sum}
${types}
${types}:123
`;
      return expect(compareSums(Buffer.from(sumfile))).to.eventually.deep.equal({
        matched: 1,
        unmatched: [types],
        additional: [testProto.filepath],
        errors: {
          [types]: {
            code: '',
            details: '',
            message: `should specify sum string for file "${types}".`
          }
        }
      });
    });
  ...
  describe('.upload()', function () {
    ...
    beforeEach(function () {
      ...
      uploadCall = client.upload(function (err, res) {
    ...
    afterEach(async function () {
      try {
        fs.unlinkSync(path.join(staticProtoRoot, testProto.filepath));
      } catch (e) {
        // skip
      }
      ...
      while (dirpath !== '.') {
        try {
          fs.rmdirSync(path.join(staticProtoRoot, dirpath));
        } catch (e) {
          // skip
        }
    ...
    it('should add new file after file uploaded', async function () {
      const exists = (flag) =>
        expect(fs.existsSync(path.join(staticProtoRoot, testProto.filepath))).to.equal(flag);
      exists(false);
      uploadCall.end({
        files: {
          [testProto.filepath]: {
            content: testProto.content,
            sum: testProto.sum
          }
        }
      });

      await expect(resultPromise).to.eventually.deep.equal({success: 1, update: 1, errors: {}});
      exists(true);
    });

    it('should ignore newest file', async function () {
      const exists = (flag) =>
        expect(fs.existsSync(path.join(staticProtoRoot, filepath))).to.equal(flag);
      exists(true);
      uploadCall.write({
        files: {
          [filepath]: {
            content: expectedFileBuf,
          }
        }
      });
      uploadCall.end();
      await expect(resultPromise).to.eventually.deep.equal({success: 1, update: 0, errors: {}});
      exists(true);
    });

    it('should support to upload multiple times in one call', async function () {
      const exists = (flag) =>
        expect(fs.existsSync(path.join(staticProtoRoot, testProto.filepath))).to.equal(flag);
      exists(false);
      uploadCall.write({
      ...
      uploadCall.end();
      await expect(resultPromise).to.eventually.deep.equal({success: 2, update: 1, errors: {}});
      exists(true);
    });

    it('should support to upload multiple files in one time', async function () {
      const exists = (flag) =>
        expect(fs.existsSync(path.join(staticProtoRoot, testProto.filepath))).to.equal(flag);
      exists(false);
      uploadCall.write({
